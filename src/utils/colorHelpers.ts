/**
 * Color utilities for event styling
 */
...
/**
 * Get a darker version of a color for text (Apple Calendar style)
 * Reduces lightness significantly to ensure readability
 */
export function getDarkerTextColor(hexColor: string): string {
  ...
  const darkerRgb = hslToRgb(hsl.h, darkerS, darkerL);
  return rgbToHex(darkerRgb.r, darkerRgb.g, darkerRgb.b);
}

/**
 * Apple Calendar inspired color palette
 * These are softer, more pastel versions similar to what Apple uses
 */
...
/**
 * Get a random color from the Apple palette
 */
